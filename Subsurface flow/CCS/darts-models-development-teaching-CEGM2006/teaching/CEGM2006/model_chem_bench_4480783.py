...
class Model(DartsModel):
    ...
    def set_physics(self, grid_1D, res, custom_physics, verbose: bool = False):
        ...
        if grid_1D:
            ...
            super().set_reservoir(reservoir, verbose=verbose)
        ...
        physics.add_property_operators(CustomPropertyOperators(physics.vars, property_container))

        return super().set_physics(physics, verbose=verbose)

    def set_wells(self, grid_1D, verbose: bool = False):
        if grid_1D:
            """well location"""
            self.reservoir.add_well("INJ_GAS")
            self.reservoir.add_perforation(well_name="INJ_GAS", cell_index=(1, 1, 1), multi_segment=False)
            ...
            for ii in range(self.ny):
                ...
                self.reservoir.add_perforation(well_name="PROD_" + str(1), cell_index=(self.nx, 1, ii + 1),
                                               multi_segment=False)

        return super().set_wells(verbose=verbose)

    # Initialize reservoir and set boundary conditions:
    def set_initial_conditions(self):
        """ initialize conditions for all scenarios"""
        self.initial_values = {vars: self.ini_comp[i] for i, vars in enumerate(self.physics.vars[1:self.physics.nc])}
        self.initial_values['pressure'] = self.init_pres
